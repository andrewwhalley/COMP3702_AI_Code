...
public class NNClassifier extends LetterClassifier {
  ...
  public double[] test(Bitmap map) {
	// Use preprocessing
//	Bitmap pmap = Preprocessor.scaleBitmap(map);
//	double[] out=nn.feedforward(pmap.toDoubleArray());
  ...
  public void train(ClassifiedBitmap[] maps, int nPresentations, double eta) {
	  ...
	  for (int p=0; p<nPresentations; p++) {
	      ...
	      rmse = nn.train(((Bitmap)maps[sample]).toDoubleArray(), targets[maps[sample].getTarget()], eta);
	  }
  }
